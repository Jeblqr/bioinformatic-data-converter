...
def example_basic_conversion():
    ...
    result = convert_single_file(filename="test_data/genomics_gwas.tsv", verbose=True)
...
def example_batch_conversion():
    ...
    results = convert_multiple_files(
        file_list=files, keep_unmatched=False, verbose=False
    )
...
def example_with_report():
    ...
    # Convert
    result = convert_single_file(filename=input_file, verbose=True)
    ...
    report.set_output_info(
        filename="output_metabolomics.tsv", columns=list(result.columns)
    )

    # Set mapping info
    mapping = auto_suggest_mapping(df_orig)
    report.set_column_mapping(mapping=mapping, unmapped=[])
